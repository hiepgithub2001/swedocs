...
Alongside the knowledge areas there is a second collection, for distillations of
real, published books. A book here carries `collection: Book summaries` in its
README frontmatter, which is what puts it on its own tab in the reader.
...
checked against it; a summary written from memory is a guess with a confident
voice, and there is no way to tell from the inside which parts are wrong.
